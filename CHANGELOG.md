...
All notable changes to WiTech are documented in this file.
...
- Added `WiTech` as a globally registered Termux command via `$PREFIX/bin`
- `WiTech` — run with default settings (wlan0 + Pixie Dust)
- `WiTech menu` — open interactive menu without triggering auto-attack
- `WiTech old` — run the legacy v1 engine (w1.py) for compatibility
- `WiTech update` — pull latest updates from GitHub and re-apply setup
- `WiTech help` — open the built-in interactive help guide
- `WiTech fix` — launch the root/superuser repair script
- `WiTech contact` — open the developer contact menu
...
- `WiTech update` internally re-runs `install.sh` to keep the command fresh

### Built-in Help Guide (`help.py`)

- Added `help.py` — a full interactive help guide with 9 sections
- Section 1: What is WiTech — introduction and overview
...
- Section 9: Quick reference for all `WiTech` commands
